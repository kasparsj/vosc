...
class BaseTex : public ofBaseHasTexture, public ofBaseHasPixels {
public:
    static shared_ptr<BaseTex> factory(string type, string name, const vector<float>& args);
    static shared_ptr<BaseTex> factory(string source, const vector<float>& args);

    BaseTex(string path, const vector<float>& args) : path(path), args(args) {}
    virtual ~BaseTex() = default;
    ...
    virtual ofTexture& getTexture() override {
        throw "abstract function BaseTex::getTexture called";
    }
