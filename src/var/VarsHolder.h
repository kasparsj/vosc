...
public:
    ...
    template <typename T>
    shared_ptr<Variable<T>> setVar(const string& name, T value);
    template <typename T>
    shared_ptr<Variable<T>> setVar(const string& name, vector<T> value);
    shared_ptr<Variable<float>> setVar(const string& name, bool value);
