...
void main() {
    ...
    if (uv.x < 0.5 && uv.y < 0.5) {
        // Top-left: original
        fragColor = texture(srctex, quadrantUV);
    }
    else if (uv.x >= 0.5 && uv.y < 0.5) {
        // Top-right: horizontal mirror
        fragColor = texture(srctex, vec2(1.0 - quadrantUV.x, quadrantUV.y));
    }
    else if (uv.x < 0.5 && uv.y >= 0.5) {
        // Bottom-left: vertical mirror
        fragColor = texture(srctex, vec2(quadrantUV.x, 1.0 - quadrantUV.y));
    }
    else {
        // Bottom-right: both horizontal and vertical mirror
        fragColor = texture(srctex, vec2(1.0 - quadrantUV.x, 1.0 - quadrantUV.y));
    }
}
